...
const IMGHDR imgPointer = {5,5,0x1,(uint8_t*)Pointer};
...
char header[128];
int filelist_menu_onkey(GUI *gui, GUI_MSG *msg)
{
  FVIEW *fview=MenuGetUserPointer(gui);
  FLIST *fl;
  int i, n;
...
void filelist_menu_ghook(GUI *gui, int cmd)
{
  FVIEW *fview=MenuGetUserPointer(gui);
  if (cmd==3)
  {

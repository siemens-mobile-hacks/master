...
void MakeFileName(char *dest, const char *patch_id) {
	char file_name[16];
	sprintf(file_name, "00000000");
	int len = MIN(strlen(patch_id), 8);
	strncpy(file_name + (8 - len), patch_id, len);
	strcat(file_name, ".ptc");
...
void initPatchConfig(PTC_CONFIG *ptcfg)
{
	...
	strcpy(ptcpath, PATCH_DIR);
	strcat(ptcpath, PTC_FOLDR);
	MakeFileName(ptcid, ptcfg->patchInfo->patchID);
	strcat(ptcpath, ptcid);

...
void option_menu_ghook(GUI *gui, int cmd)
{
	if (cmd==0x0A)
	{
		DisableIDLETMR();
...
void deleteThis(void)
{
	...
	if(ptcfg)
	{
		FSTATS fstats;
		unsigned int err;
		sprintf(path, "%s%s%8s", PATCH_DIR, PTC_FOLDR, ptcfg->patchInfo->patchID);
...
int profile_menu_onkey(GUI *gui, GUI_MSG *msg)
{
	PTC_CONFIG *ptcfg;
	int c=GetCurMenuItem(gui);
		...
		case RIGHT_SOFT:
			{
				...
				if(ptcfg)
				{
					...
					SetMenuItemIcon(gui, c, getPatchOnOff(ptcfg, c));
					REDRAW();
				}
				return (-1);
			}
...
void profile_menu_ghook(GUI *gui, int cmd)
{
	if (cmd==0x0A)
		DisableIDLETMR();
}
